...
import { Plus, Trash2, Sliders, Zap, Edit2, Check } from 'lucide-react';
...
function Harm() {
  ...
  const chartData = useMemo(() => {
    const points = [];
    const pointsCount = 300; // Resolução expandida para cobrir os 3 ciclos com suavidade
    const f = 60;
    const totalTime = 3 * (1 / f); // 3 períodos completos (~0.050 segundos ou 50ms)
    ...
    for (let i = 0; i <= pointsCount; i++) {
      ...
      points.push({
        time: parseFloat((t * 1000).toFixed(2)), // Converte para milissegundos (0 a 50ms)
  ...
  return (
    <div style={{ padding: '24px', fontFamily: 'Arial, sans-serif', backgroundColor: '#f8fafc', minHeight: '100vh' }}>
      <h2 style={{ textAlign: 'center', color: '#0f172a', marginBottom: '24px', fontWeight: 'bold' }}>SINTETIZADOR E ANALISADOR DE HARMÔNICAS</h2>

      <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap', maxWidth: '1300px', margin: '0 auto' }}>
        ...
        <div style={{ flex: '1', minWidth: '360px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ backgroundColor: '#ffffff', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', flex: 1, maxHeight: '420px', overflowY: 'auto' }}>
                ...
                <div key={h.order} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', backgroundColor: h.order === 1 ? '#eff6ff' : '#f8fafc', borderRadius: '6px', borderLeft: h.order === 1 ? '4px solid #2563eb' : '4px solid #64748b' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <button onClick={() => handleEditClick(h)} title="Editar parâmetro" style={{ border: 'none', background: 'none', cursor: 'pointer', padding: '4px', color: '#475569' }}>
                      <Edit2 size={14} className="hover:text-blue-600" />
                    </button>
        ...
        <div style={{ flex: '2', minWidth: '500px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {/* PLOT DA FORMA DE ONDA RESULTANTE (3 CICLOS COM MARCAS LINEARES ESPAÇADAS) */}
          <div style={{ backgroundColor: '#ffffff', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', height: '420px', display: 'flex', flexDirection: 'column' }}>
            <h3 style={{ margin: '0 0 12px 0', fontSize: '15px', color: '#334155', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <Zap size={18} color="#ea580c" /> Forma de Onda Resultante (3 Ciclos - 60 Hz)
            </h3>
            ...
            <div style={{ flex: 1, width: '100%' }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 10, right: 20, left: -10, bottom: 5 }}>
                  ...
                  {/* EIXO X: Linear, marcas fixas de 5 em 5 ms */}
                  <XAxis 
                    type="number"
                    dataKey="time" 
                    scale="linear"
                  ...
                  {/* EIXO Y: Escala Linear Simétrica Baseada no Pico Máximo Calculado */}
                  <YAxis 
                    type="number"
                    scale="linear"
                    domain={[-yMaxLimit, yMaxLimit]}
                    tickCount={9} // Divide o espaço vertical de forma idêntica e simétrica
                    stroke="#64748b"
                    label={{ value: 'Amplitude Instantânea (V)', angle: -90, position: 'insideLeft', offset: 15, fill: '#64748b', fontSize: '12px', fontWeight: 'bold' }}
                  />
